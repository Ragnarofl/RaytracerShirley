...
class Isotropic : public Material {
	public:
		...
		virtual bool scatter(Ray& r_in, const hit_record& rec, vec3& attenuation, Ray& scattered, std::mt19937& mt) const {
			scattered = Ray(rec.p, random_in_unit_sphere(mt));
...
class ConstantMedium : public Hitable {
	public:
		...
		virtual bool hit(Ray& r, float t_min, float t_max, hit_record& rec, std::mt19937& mt) const {
			...
			bool db = enableDebug && (random_unit_float(mt) < 0.0001);

			hit_record rec1, rec2;
			if (_boundary->hit(r, -FLT_MAX, FLT_MAX, rec1, mt)) {
				if (_boundary->hit(r, rec1.t + 0.0001, FLT_MAX, rec2, mt)) {
					if (db) std::cerr << "\nt0 t1" << rec1.t << " " << rec2.t << "\n";
					...
					float hit_distance = -(1 / _density) * log(random_unit_float(mt));
